...
class ClusterEncode(nn.Module):
    ...
    def nce_loss_kmeans(self, x, centroids):
        ...
        return proto_nce_loss_x, proto_nce_loss
    ...
    def forward(self, x, centroids):
        ...
        if self.mode == 'k-means':
            b,v,d = x.shape
            raw_loss, loss = self.nce_loss_kmeans(x, centroids)
        ...
        loss_info = {
            'raw_proto_nce_loss': raw_loss,
            'scaled_proto_nce_loss': loss,
        }
        return loss, aug_samples, loss_info
...
class ContextEncoder(nn.Module):
    ...
    def get_context(self, mask_emb, history_emb, neigh_emb):
        ...
        if self.mode == 'concat':
            ...
            context_cro = torch.cat([mask_emb, neigh_emb], dim=1)
            context_fin = torch.cat([mask_emb, history_emb], dim=1)
            # return context_emb
            return context_cro, context_fin
        else:
            raise NotImplementedError
    ...
    def forward(self, x, batch_centroids_emb,  **kwargs):
        ...
        neigh_loss, neigh_emb, loss_info = self.cluster(x, batch_centroids_emb) # B,3,D 和B, 3, D

        # context_emb = self.get_context(mask_emb, history_emb, neigh_emb)
        context_cro, context_fin = self.get_context(mask_emb, history_emb, neigh_emb)

        # return neigh_loss, context_emb
        return neigh_loss, context_cro, context_fin, loss_info
...
#     def get_context(self, mask_emb, neigh_emb):
#         # 获取对应的context emb
#         # neigh_emb = torch.zeros_like(neigh_emb) # aba
#         if self.mode == 'concat':
#             context_emb = torch.cat([mask_emb, neigh_emb], dim=1) # B, 9 （3+3+3+1）, D
#             return context_emb
#         else:
#             raise NotImplementedError
...
#         context_emb = self.get_context(mask_emb, neigh_emb)
