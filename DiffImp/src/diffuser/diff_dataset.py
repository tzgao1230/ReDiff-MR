...
class CustomDataset(Dataset):
    ...
    def get_contexts(self, num):
        ...
        patient_id, visit_id = [], []
        conditions, procedures, drugs, note = [], [], [], []
        mask, mask_hist = [], []
        conditions_hist, procedures_hist, drugs_hist, note_hist = [], [], [], []
        conditions_comp, procedures_comp, drugs_comp, note_compt = [], [], [], []
        for i in range(num):
            patient_id.append(self.patient_id[i])
            visit_id.append(self.visit_id[i])
            conditions.append(self.samples_static[i][0]) # 应该放完整的
            procedures.append(self.samples_static[i][1])
            drugs.append(self.samples_static[i][2])
            ...
            procedures_hist.append(self.samples_history[i][1])
            drugs_hist.append(self.samples_history[i][2])
            ...
            procedures_comp.append(self.samples_static[i][1])
            drugs_comp.append(self.samples_static[i][2])
    ...
    def __getitem__(self, index):
        data_index = {
            ...
            'procedures': self.samples_static[index][1],
            'drugs': self.samples_static[index][2],
            ...
            'procedures_hist': self.samples_history[index][1],
            'drugs_hist': self.samples_history[index][2],
            ...
            'procedures_comp': self.samples_static[index][1],
            'drugs_comp': self.samples_static[index][2],
...
class CFGDataset(Dataset):
    ...
    def get_contexts(self, num):
        ...
        mask, mask_hist = [], []
        conditions_hist, procedures_hist, drugs_hist, note_hist = [], [], [], []
        conditions_comp, procedures_comp, drugs_comp, note_comp = [], [], [], []
        for i in range(num):
            patient_id.append(self.patient_id[i])
            visit_id.append(self.visit_id[i])
...
class MIIIDataset(DatasetFactory):
    def __init__(self, samples, aug, cfg=False, p_uncond=None, **kwargs):
        ...
        self.empty_context = torch.randn(6, kwargs['config'][
            'dim'])  # for classifier free guidance 9= 3mask + 3 condition history + 3 prototype
        self.p_uncond = p_uncond
        if aug:                 # 是否要增强数据的意思，大概，在第一轮训练阶段这里是关闭的
        ...
        self.mask_contexts, self.his_contexts = None, None      # 初始化掩码上下文和历史上下文，后续大概会用到
    ...
    def generate_mask(self, sample_static, m=3, mask_num=1):
        """[list, list, list]"""
        """
        tianzhi add 这里随机生成掩码，mask掉样本中其中一个view，并且返回掩码和掩码后的样本
        """
    ...
    def transform_aug_dataset(self, dataset):
        ...
        for data in dataset:
            ...
            visit_static = [data['conditions_raw'], data['procedures_raw'], data['drugs']]
            ...
            if visit_length < 2: # 这里似乎也可以修改为一样的pad即可，不然太浪费数据了。
                ...
                visit_history = [data['conditions'][:visit_length], data['procedures'][:visit_length], data['drugs_hist'][:visit_length]]
    ...
    def transform_dataset(self, samples, cfg=False):
        ...
        samples = get_last_visit_sample(samples)        # tianzhi add 这里获取了每个患者的最后一次访问记录，4500个患者好像
        patient_id = []
        visit_id = []
        samples_static = []
        mask_sample_static = [] # mask后的数据
        ...
        sample_mask = []
        sample_history_mask = []
        for _, patient in samples.items(): # 转为visit-level  # tianzhi add 这里的samples是一个字典，key是患者id，value是患者的所有访问记录
            visit_length = len(patient['conditions'])       # tianzhi add 这里应该是说的一个患者所有的访问次数
            for visit in range(visit_length):
                if sum(patient['incomplete'][visit])==0:        # tianzhi add 如果每一次访问的视图没有缺失，那么就随机mask掉三个视图中其中一个
                    patient_id.append(patient['patient_id'])
                    visit_id.append(patient['visit_id'])
                    visit_static = [patient['conditions'][visit], patient['procedures'][visit], patient['drugs_hist'][visit]] # [[list, list, list]]
                    samples_static.append(visit_static)
                    ...
                    if visit <2:            # tianzhi add 如果当前访问是患者的前两次访问
                        visit_his_cond, visit_his_drug, visit_his_proc = [['<pad>']],[['<pad>']],[['<pad>']]
                        visit_his_cond_m, visit_his_drug_m, visit_his_proc_m = [True], [True], [True]
                        samples_history.append([visit_his_cond, visit_his_drug, visit_his_proc])
                        sample_history_mask.append([visit_his_cond_m, visit_his_drug_m, visit_his_proc_m])
                    else:
                        samples_history.append([patient['conditions'][:visit], patient['procedures'][:visit], patient['drugs_hist'][:visit]])
...
class MIV_NoteDataset(DatasetFactory):
    ...
    def transform_aug_dataset(self, dataset):
        ...
        for data in dataset:
            ...
            visit_static = [data['conditions_raw'], data['procedures_raw'], data['drugs'], data['note'][-1]] # static是4个 hist也是4个，但是mask是3个
            # visit_static = list(map(list, visit_static))
            samples_static.append(visit_static)
            mask_visit = visit_static.copy()
            ...
            if visit_length < 2:  
                ...
                visit_history = [data['conditions'][:visit_length], data['procedures'][:visit_length], data['drugs_hist'][:visit_length], data['note'][:visit_length]]
                samples_history.append(visit_history)
                sample_history_mask.append(data['incomplete'][:visit_length]) # 这样方便代码书写,包含自身
        ...
        for _, patient in samples.items(): # 转为visit-level
            ...
            for visit in range(visit_length):
                if sum(patient['incomplete'][visit])==0:
                    ...
                    visit_id.append(patient['visit_id'])
                    # 不需要真的mask
                    visit_static = [patient['conditions'][visit], patient['procedures'][visit], patient['drugs_hist'][visit], patient['note'][visit]] # [[list, list, list]]
                    samples_static.append(visit_static)
                    ...
                    if visit <2:
                        ...
                        samples_history.append([patient['conditions'][:visit], patient['procedures'][:visit], patient['drugs_hist'][:visit], patient['note'][:visit]])
                        sample_history_mask.append(patient['incomplete'][:visit]) # 这样方便代码书写,包含自身
                else:
                    # 没法检验，对于那些缺失的visit
                    continue
...
def get_dataset(name, aug, **kwargs):
    ...
    p_uncond = kwargs['config']['dataset'][name]['p_uncond']        # 这两个参数是干什么的呢？？？？？
    if name == 'MIII':
