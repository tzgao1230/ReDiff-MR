...
path = '/home/qluai/tianzhi1/MMHealth/data/'
code_to_group, group_to_id, id_to_group = create_label_for_phenotyping(path)

# basline别的数据集注释掉if，不然报错。
note_dict = get_note(config, "/mnt/tianzhiData/MIMICIV/") if config['DATASET'] =='MIV-Note' else None
...
def generate_patient_group(samples, path):
    ...
    for record in last_visits:
        ...
        max_num = len(incomplete) * 3           # max_num = 3
...
############################################################################
# drug_recom_miii_fn MIMIC3药物推荐fn
def drug_recommendation_miii_fn(patient:Patient):
    ...
    for i in range(len(patient)-1):
        ...
        next_conditions = next_visit.get_code_list(table="DIAGNOSES_ICD")
        next_procedures = next_visit.get_code_list(table="PROCEDURES_ICD")
        next_drugs = next_visit.get_code_list(table="PRESCRIPTIONS")
        # ATC 3 level
        drugs = [drug[:4] for drug in drugs]
        next_drugs = [drug[:4] for drug in next_drugs]
        """
        # exclude: visits without condition, procedure, or drug code
        # if len(conditions) * len(procedures) * len(drugs) == 0:
        #     continue
        这里处理的时候直接跳过了有视图缺失的情况，而我们处理的时候是需要保留患者完整的访问记录的
        """

        # 过滤掉三种视图都没有的情况。这里处理数据的时候并没有打label
        filter_res = filter_lists_for_drug(conditions, procedures, drugs, next_drugs,next_conditions, next_procedures)
        if filter_res is None:
            continue
        else:
            conditions, procedures, drugs, next_drugs, mask = filter_res
        ...
        samples.append(
            {
                ...
                "drugs": drugs,
                "drugs_hist": drugs,
                "visit_id_hist": visit.visit_id,
                "conditions_raw": conditions,
                "procedures_raw": procedures,
                "incomplete_raw": mask,  # [False, False, False], 被mask掉的地方为True，即缺失的地方为True
                "next_conditions": next_conditions,
                "next_procedures": next_procedures,
                "labels": next_drugs,
            }
        )
...
def drug_recommendation_miv_fn(patient:Patient):
    samples = []
    for i in range(len(patient)-1):
        visit: Visit = patient[i]
        next_visit: Visit = patient[i + 1]
...
def drug_recommendation_eICU_fn(patient:Patient):
    ...
    for i in range(len(patient)-1):
        ...
        next_conditions = next_visit.get_code_list(table="diagnosis")
        next_procedures = next_visit.get_code_list(table="physicalExam")
        next_drugs = next_visit.get_code_list(table="medication")
...
def filter_lists_for_drug(A, B, C, label, conditions, procedures): # 对retain-grasp这些baseline，这里用unk而不是pad
    # Check if D is empty
    if isinstance(label, list) and len(label) == 0:
        return None  # or return empty lists if preferred
    # Check if A, B, and C are all empty
