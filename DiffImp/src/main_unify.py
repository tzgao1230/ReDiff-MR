...
from utils import split_by_patient, load_pickle, save_pickle, set_random_seed, get_tokenizers, get_ddi_matrix, get_all_tokenizers
...
def run_single_config(augment=False, retrain=False, exp_num='3', pretrain=False):
    ...
    # load datasets
    # STEP 1: load data
    root_to = '/home/qluai/tianzhi1/MMHealth/data/{}/{}/processed/'.format(config['TASK'], config['DATASET'])

    task_fn, mode = get_task_fn(config)         # 这里的task_fn很重要，关系到数据怎么处理。如果需要应用到自己的任务上，需要重写这里的task_fn
    if not os.path.exists(root_to + 'datasets_pre_stand.pkl'):
        print("Prepare dataset!")
        if config['DATASET'] == 'MIII':
            base_dataset = MIMIC3Dataset(
                root="/mnt/tianzhiData/MIMICIII",
        ...
        if config['DEV']:
            ...
            try:
                reserved_tensor = torch.ones((10, 10)).to('cuda:0')  # 占据GPU
                print("GPU Memory Usage", torch.cuda.memory_allocated('cuda:0') / 1024 / 1024 / 1024, "GB")
                ...
                if os.path.exists(root_to + 'train_dataset.pkl'):
                    ...
                    train_samples = load_pickle(root_to + 'train_dataset.pkl')      # tianzhi add 7456条数据
    ...
    if augment:
        ...
        diff_config['device'] = 0   # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        diff_config['force_restart'] = retrain
        if config['DATASET'] == "MIV-Note": # 要改train_diff, eval_diff
            diff_config['nnet']['num_contx_token'] = 10
        tokenizers = get_tokenizers(sample_dataset)     # tianzhi add 这里就是说对于所有的数据进行tokenizer处理，获得相应的词表（diag、proce、med）
        ...
        if retrain or not os.path.exists(diff_config['ckpt_root']+ config['DATASET']):
            ...
            train_samples, test_samples = augment_dataset(generated_model,diff_config, config,  [train_dataset, test_dataset], tokenizers, root_to=root_to)
