...
def get_all_tokenizers(dataset, special_tokens=False):
    if not special_tokens:
        special_tokens = ["<pad>", "<unk>"] # 把pad取消
    feature_keys = ["conditions", "procedures", "drugs"]
    feature_tokenizers = {}
...
# tianzhi add
# 获得DDI矩阵
def get_ddi_matrix(med_voc, ddi_file):

    TOPK = 40 # topk drug-drug interaction
    cid2atc_dic = defaultdict(set)          # tianzhi add CID编码到ATC编码映射，用于存储
    med_voc_size = len(med_voc.idx2token)    # 词表大小
    med_unique_word = [med_voc.idx2token[i] for i in range(med_voc_size)]    # 所有的药物的ATC4
    atc3_atc4_dic = defaultdict(set)
    for item in med_unique_word:
        atc3_atc4_dic[item[:4]].add(item)   # tianzhi add 把ATC4转换为ATC3
    
    with open("/home/qluai/hgc/tianzhi/COGNet/data/drug-atc.csv", 'r') as f:
        for line in f:
            ...
            for atc_j in cid2atc_dic[cid2]:
                ...
                for i in atc3_atc4_dic[atc_i]:
                    for j in atc3_atc4_dic[atc_j]:
                        if med_voc.token2idx[i] != med_voc.token2idx[j]:
                            ddi_adj[med_voc.token2idx[i], med_voc.token2idx[j]] = 1
                            ddi_adj[med_voc.token2idx[j], med_voc.token2idx[i]] = 1
    output_path = "/root/autodl-tmp/MMHealth/data/ddi_A_final.pkl"
    dill.dump(ddi_adj, open(output_path, 'wb')) 
    print("complete!!!!!")
    return ddi_adj