...
parser.add_argument(
    ...
    type=float,
    default=0.1,
    help="fixed fusion weight for the EHR-DDI graph branch, must be between 0 and 1",
)
...
def eval(model, data_eval, voc_size, epoch): 
    ...
    for step, input in enumerate(data_eval):
        ...
        for adm_idx, adm in enumerate(input):
            ...
            target_drug_flag = adm[3][2]
            if target_drug_flag == 1:
                continue
...
def main():
    ...
    ehr_adj = "../data/mimic-iii/ehr_adj_final.pkl"
    ...
    with open(ehr_adj, "rb") as file:
        ehr_adj = dill.load(file)
    ...
    model = SafeDrugModel(
        ...
        emb_dim=args.dim,
        device=device,
        graph_branch_init=args.graph_branch_init,
    )
    # model.load_state_dict(torch.load(open(args.resume_path, 'rb')))

    if args.Test:
        model.to(device=device)
        model.build_retrieval_bank(data_train)
        model.load_state_dict(torch.load(open(args.resume_path, "rb")))
        # model.to(device=device)
        tic = time.time()
    ...
    for epoch in range(EPOCH):
        ...
        for step, input in enumerate(data_train):
            ...
            for idx, adm in enumerate(input):
                ...
                if current_ddi_rate <= args.target_ddi:
                    ...
                    beta = min(1.0, max(0.0, 1 + (args.target_ddi - current_ddi_rate) / args.kp))
                    loss = (
        ...
        graph_branch_weight = model.graph_branch_weight
        print(
            "training time: {}, test time: {}, graph_branch_weight: {:.4f}".format(
                time.time() - tic, time.time() - tic2, graph_branch_weight
            )
        )
